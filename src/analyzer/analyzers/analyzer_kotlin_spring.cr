...
class AnalyzerKotlinSpring < Analyzer
  ...
  def analyze
    ...
    Dir.glob("#{@base_path}/**/*") do |path|
      ...
      if File.directory?(path)
        ...
        # Load Kotlin file content into cache for processing
        content = File.read(path, encoding: "utf-8", invalid: :skip)
        FILE_CONTENT_CACHE[path] = content
        parser = create_parser(Path.new(path))
        ...
        if has_spring_bindings
          ...
          parser.import_statements.each do |import_statement|
            ...
            if import_path.ends_with?("/*")
              ...
              source_path = root_source_directory.join(import_path + EXTENSION)
                ...
                if method_annotation.name.ends_with? "Mapping"
                  ...
                  if method_annotation.name == "@RequestMapping"
                    ...
                    if !mapping_index.nil?
                      line = method_annotation.tokens[0].line
                      request_method = mapping_annotations[mapping_index][1..-8].upcase
  ...
  def get_mapping_path(parser : KotlinParser, tokens : Array(Token), method_params : Array(Array(Token)))
