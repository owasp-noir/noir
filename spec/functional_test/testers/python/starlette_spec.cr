...
expected_endpoints = [
  Endpoint.new("/", "GET"),
  Endpoint.new("/users/{user_id}", "GET", [Param.new("user_id", "", "query")]),
  ...
  Endpoint.new("/profile/{name}", "GET", [Param.new("name", "", "query")]),
  Endpoint.new("/api/items", "GET"),
  Endpoint.new("/api/items/{id}", "GET", [Param.new("id", "", "query")]),
]
