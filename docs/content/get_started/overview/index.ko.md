...
업데이트되지 않은 문서나 프록시 로그에 의존하는 대신, Noir는 실제 코드를 기반으로 완전한 공격 표면(Attack Surface) 지도를 생성합니다. 이 데이터는 DAST와 연동하여 DevSecOps 파이프라인 내에서 테스트되지 않고 방치되는 엔드포인트가 없도록 보장합니다.
