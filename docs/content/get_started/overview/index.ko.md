...
**AI SAST를 진짜 공격 표면으로 가리킵니다.** Noir가 추출한 엔드포인트 인벤토리(진입점, 소스 파일, 파라미터, 태그)는 LLM 기반 SAST, 코드 감사, 보안 에이전트가 공격자에게 도달 가능한 코드의 버그를 찾는 데 필요한 바로 그 컨텍스트입니다. 모델에게 저장소 전체를 훑게 하지 말고, Noir가 미리 매핑해 둔 공격 표면을 그대로 넘기세요.
