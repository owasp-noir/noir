...
앞의 여섯 행은 앱 번들에 선언되는 **클라이언트** 측 연결이고, 마지막 두 행은 **서버** 측, 즉 OS가 해당 호스트의 URL에 대해 앱을 열도록 호스트가 게시하는 well-known 파일입니다. 둘 다 동일한 `universal-link` protocol과 출력 모델을 통해 처리됩니다.
