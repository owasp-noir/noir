...
description = "AI 기반 소스 코드 리뷰에 raw 코드 대신 구조화된 보안 context를 흘려보내는 두 v1 플래그 이야기."
date = "2026-05-23"
...
요즘 AI 기반 소스코드 분석 도구가 많아졌습니다. 다들 코드를 입력으로 받아서 LLM에 던지고 결과를 받는 패턴이죠. 그런데 실제로 잘 동작하려면 LLM이 "무엇을 봐야 하는지"를 알려주는 게 핵심입니다. 그냥 codebase를 통째로 던지면 token만 태우고, 정작 봐야 할 지점은 놓치는 경우가 많습니다.
...
같은 Flask `/sign` POST endpoint를 `--ai-context`로 돌리면 (실제 v1.0.0 출력에서 발췌):
...
1.x 라인 동안엔 패턴 카탈로그를 더 framework-aware하게 확장하고, signal 종류를 추가하는 방향으로 진행할 예정입니다. 피드백이나 새 패턴 제안은 [GitHub Issues](https://github.com/owasp-noir/noir/issues)로 환영합니다.
