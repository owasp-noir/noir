...
이슈 #2353 에서 엔진들에 `@result_mutex` 와 `append_endpoint` 헬퍼가 추가되었다가 #2360 에서 다시 제거되었습니다. single-threaded 빌드에서는 발생할 수 없는 race 를 막고 있었고, 일부 엔진에만 적용되어 있었으며, 이 문서와 정면으로 어긋났기 때문입니다. Mutex 를 다시 넣으려 한다면 그 반복을 여는 셈입니다. MT 를 먼저 켜고, 동기화는 `parallel_analyze` 레이어에 한 번만 추가하세요.
